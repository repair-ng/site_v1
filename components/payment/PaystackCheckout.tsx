...
export function PaystackCheckout({
  ...
  async function startCheckout() {
    ...
    try {
      ...
      if (data.demo) {
        // No PAYSTACK_SECRET_KEY configured yet — see
        // /app/api/payment/initiate/route.ts. Lets you click through the
        // whole flow before real credentials exist.
        setStatus("demo");
        return;
      }

      // A full-page redirect to Paystack's hosted checkout loses React
      // state, so stash what /payment/callback needs to finish the job
      // (build the WhatsApp message) once the customer is redirected back.
      sessionStorage.setItem(
        `repair:${reference}`,
        JSON.stringify({ reference, customer, metadata })
      );

      // Real Paystack Checkout: send the browser to the hosted payment page.
  ...
  return (
    <div>
      ...
      {status !== "demo" ? (
        ...
        >
          {status === "starting" ? "Starting checkout..." : "Pay with Paystack"}
        </button>
      ) : (
        <div className="mt-4 rounded-ticket border border-signal/30 bg-signal/5 px-4 py-3">
          <p className="text-xs font-medium text-signal">
            DEMO MODE — no PAYSTACK_SECRET_KEY is set in this deployment yet,
            so this simulates a successful payment instead of opening a real
            checkout. Add your Paystack secret key in the environment
